...
std::vector<Coord> Pawn::calc_valid_moves(){
	...
	for(int i=0; i<4; i++){
		if(!(possible_move[i].x + posn.x > 7 || possible_move[i].x + posn.x < 0 || possible_move[i].y + posn.y > 7 || possible_move[i].y + posn.y < 0)){
			...
			if(i==0 && board->get_piece(Coord(possible_move[0].x + posn.x, possible_move[0].y + posn.y))==nullptr && (is_pinned()==-1 || is_pinned()==0)){
			...
			else if(i==1 && board->get_piece(Coord(possible_move[1].x + posn.x, possible_move[1].y + posn.y))!=nullptr){
			...
			else if(i==2 && board->get_piece(Coord(possible_move[2].x + posn.x, possible_move[2].y + posn.y))!=nullptr){
			...
			else if(i==1 && board->get_piece(Coord(posn.x+1, posn.y))->get_name_value()==1 && (is_pinned()==-1 || is_pinned()==1)){
				if(board->get_piece(Coord(posn.x+1, posn.y))->get_en_passant()){
					valid.push_back(Coord(possible_move[1].x + posn.x, possible_move[1].y + posn.y));
				}
			}
			else if(i==2 && board->get_piece(Coord(posn.x-1, posn.y))->get_name_value()==1 && (is_pinned()==-1 || is_pinned()==3)){
				if(board->get_piece(Coord(posn.x-1, posn.y))->get_en_passant()){
					valid.push_back(Coord(possible_move[2].x + posn.x, possible_move[2].y + posn.y));
				}
			}
			...
			else if(i==3 && first_move == true && board->get_piece(Coord(posn.x, 1 + posn.y))==nullptr 
