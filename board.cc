...
void Board::reset() {
	...
	for (std::vector<int>::iterator i = move_list.begin(); i != move_list.end(); ++i) {
		for (std::vector<int>::iterator j = move_list[i].destroy.begin(); j != move_list[i].destroy.end(); ++j) {
			delete move_list.destroy[j];
		}
	}
	move_list.clear();
	black_pieces.clear();
